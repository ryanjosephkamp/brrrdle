...
| **Phase 22 (active phase)** | §27 | **Prompt 1 (planning + governance) complete (`phase_id = 64`)** and **Prompt 2 (full execution) complete (`phase_id = 65`)**: timezone-aware local-midnight daily reset, balanced anti-gaming, cross-page countdown, reset alert + new unique sound, dev-mode Simulate Time tool, modular `src/daily/` service. **Addendum (§27.10) — Calendar & Countdown Positioning — incorporated as governance/planning only (`phase_id = 66`)**: Calendar as the central first-tab hub for all dailies (current + past), "Play Today's OG/GO" buttons, coin-gated past dailies (one guess permanently unlocks), countdown repositioned to the top of the UI. No calendar/countdown-repositioning source code yet; halt for explicit user approval before the addendum full-execution prompt |
...
> Status: **Governance / planning-only step complete (`phase_id = 66`)**. This step binds the new Calendar & Countdown Positioning spec into the plan, records the feature scope, the navigation/structural change, the coin-gating model, and the countdown repositioning, and updates the changelog and progress tracking. **No calendar, navigation, routing, state-management, countdown-repositioning, economy, or other source code was implemented in this step.** The finalized Phase 21 surface foundation and every existing mechanic — including the Phase 22 Prompt 2 daily-cycle work — remain 100% intact. Full execution of this addendum requires explicit user approval.
...
| 22 Addendum Prompt 2 | 67+ | Full execution (separately approved) | Calendar central daily hub (first tab) + quick-access buttons; coin-gated past dailies (one guess unlocks); full daily experience for past dailies; countdown repositioned to top (context-aware); navigation/routing/state updates; tests | Full verification matrix (§27.8 extended); responsive/accessibility smoke; halt before creating the final Phase 22 PR |

#### 27.10.8 Open Questions / Recommendations for the User

1. **Confirm the past-daily coin cost** — proposed **60 coins** (range 50–75). Confirm or override before execution.
2. **Streak semantics on the calendar** — confirm whether streaks count only the natural current-day dailies, or whether completing an unlocked past daily can also affect streak/longest-streak figures (recommended: past dailies record stats but do **not** retroactively alter streak continuity, to keep streaks meaningful).
3. **Tab removal scope** — confirm the old dedicated OG Daily / GO Daily tabs should be fully removed (vs. hidden/redirected to the Calendar). The spec says removed/integrated; recommended: remove and route any deep links to the Calendar.
4. **Calendar history depth** — confirm how far back past dailies are selectable (e.g., from launch/first recorded daily) and whether days before the user's first play are accessible.
