...
import { createPracticeOgSetup, restoreOgSession } from '../game'
import {
...
describe('multiplayer foundation', () => {
  ...
  it('checkpoints timed Practice clock ticks so elapsed time is not double-counted', () => {
    const waiting = createMultiplayerGame({
      createdAt: '2026-06-04T12:00:00.000Z',
      mode: 'og',
      playerUserIds: { 'player-one': 'host-user' },
    ...
    expect(firstTick.games[0].timeRemainingMs?.['player-one']).toBe(20_000)
    expect(firstTick.games[0].turnStartedAt).toBe('2026-06-04T12:00:10.000Z')
    expect(game.status).toBe('playing')
    expect(game.timeRemainingMs?.['player-one']).toBe(15_000)
    expect(game.turnStartedAt).toBe('2026-06-04T12:00:15.000Z')
